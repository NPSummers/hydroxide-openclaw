...
const keyringService = "hydroxide"

var ErrBridgePasswordNotFound = errors.New("stored bridge password not found")

func bridgePassFilePath() (string, error) {
	return config.Path("bridgepass.json")
}

func localStorageKey() [32]byte {
	h := sha256.New()
	io.WriteString(h, keyringService)
	io.WriteString(h, "|")
	io.WriteString(h, os.Getenv("USERPROFILE"))
	io.WriteString(h, "|")
	io.WriteString(h, os.Getenv("HOME"))
	io.WriteString(h, "|")
	io.WriteString(h, os.Getenv("USERNAME"))
	io.WriteString(h, "|")
	io.WriteString(h, os.Getenv("USER"))
	io.WriteString(h, "|")
...
func readBridgePasswords() (map[string]string, error) {
	p, err := bridgePassFilePath()
	if err != nil {
		return nil, fmt.Errorf("failed to get bridge password file path: %v", err)
	}
	...
	if os.IsNotExist(err) {
		...
		return nil, fmt.Errorf("failed to open stored bridge password file: %v", err)
	}
	defer f.Close()

	passwords := make(map[string]string)
	if err := json.NewDecoder(f).Decode(&passwords); err != nil {
		return nil, fmt.Errorf("failed to read stored bridge password file: %v", err)
	}

	return passwords, nil
}

func saveBridgePasswords(passwords map[string]string) error {
	p, err := bridgePassFilePath()
	if err != nil {
		return fmt.Errorf("failed to get bridge password file path: %v", err)
	}

	f, err := os.OpenFile(p, os.O_CREATE|os.O_WRONLY|os.O_TRUNC, 0600)
	if err != nil {
		return fmt.Errorf("failed to create stored bridge password file: %v", err)
	}
	defer f.Close()

	if err := json.NewEncoder(f).Encode(passwords); err != nil {
		return fmt.Errorf("failed to write stored bridge password file: %v", err)
	}
	if err := f.Close(); err != nil {
		return fmt.Errorf("failed to close stored bridge password file: %v", err)
	}
...
func SaveBridgePassword(username, bridgePassword string) error {
	storageKey := localStorageKey()

	encrypted, err := encrypt([]byte(bridgePassword), &storageKey)
	if err != nil {
		return err
	}

	passwords, err := readBridgePasswords()
	if err != nil {
		return err
	}
	if passwords == nil {
	...
	return saveBridgePasswords(passwords)
}

func GetStoredBridgePassword(username string) (string, error) {
	passwords, err := readBridgePasswords()
	if err != nil {
		return "", err
	}
	if passwords == nil {
		return "", ErrBridgePasswordNotFound
	}

	encrypted, ok := passwords[username]
	if !ok {
		return "", ErrBridgePasswordNotFound
	}

	storageKey := localStorageKey()
	decrypted, err := decrypt(encrypted, &storageKey)
	if err != nil {
		return "", ErrBridgePasswordNotFound
	}
...
func decodeSecretKey(password string) (secretKey [32]byte, err error) {
	passwordBytes, err := base64.StdEncoding.DecodeString(password)
	if err != nil || len(passwordBytes) != len(secretKey) {
		return secretKey, ErrUnauthorized
	}
	copy(secretKey[:], passwordBytes)
	return secretKey, nil
}

func resolveSecretKey(username, password string) (secretKey [32]byte, err error) {
	secretKey, err = decodeSecretKey(password)
	if err == nil {
		return secretKey, nil
	}
	...
	bridgePassword, err := GetStoredBridgePassword(username)
	if errors.Is(err, ErrBridgePasswordNotFound) {
		return secretKey, ErrUnauthorized
	} else if err != nil {
		return secretKey, fmt.Errorf("failed to read stored bridge password: %v", err)
	}

	return decodeSecretKey(bridgePassword)
}
...
func (m *Manager) Auth(username, password string) (*protonmail.Client, openpgp.EntityList, error) {
	secretKey, err := resolveSecretKey(username, password)
	if err != nil {
		return nil, nil, err
	}
