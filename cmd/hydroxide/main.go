...
func askBridgePass(username string) (string, error) {
	if v := os.Getenv("HYDROXIDE_BRIDGE_PASS"); v != "" {
		return v, nil
	}

	if username != "" {
		if v, err := auth.GetStoredBridgePassword(username); err == nil && v != "" {
			return v, nil
		}
	}
...
func main() {
	...
	case "auth":
		...
		if err := auth.SaveBridgePassword(username, bridgePassword); err != nil {
			log.Fatal(err)
		}

		fmt.Println("Credentials saved securely.")
		fmt.Println("For SMTP/IMAP/CardDAV, use your ProtonMail username as both username and password.")
	case "status":
	...
	case "export-secret-keys":
		...
		bridgePassword, err := askBridgePass(username)
		if err != nil {
			log.Fatal(err)
		}
